        ...
        this.state = {
            ...
            budgetOptions: [
               // { value: 'Zero', label: 'Zero spend budget' },
                ...
                { value: 'Custom', label: 'Customized Date Format' }
            ],
            options01: [
            ...
            budgetPayload: {
                ...
                Amount: "",
                Account: AWS_ACCOUNT_NUMBER || '130489330727',
                Range: `${moment(moment()).startOf('year').format('YYYY-MM-DD')}:${moment(moment()).endOf('year').format('YYYY-MM-DD')}`
...
updatebudgetAmountDataCustom() {
        const key = "Amount";
        const val = event.target.value;
      console.log(key + val)
    }
    updatebudgetNameDataCustom(){
        const key = "Name";
        const val = event.target.value;
      console.log(key + val)
    }
    ...
    componentWillReceiveProps(props) {
        ...
        if (costData) {
            ...
            if (costData && this.props.cosDetails.cosSuccess) {
                ...
                if(this.state.budgetPdfHeader == 0){

                    this.state.budgetPdfHeader.push({header:'Budget Name',dataKey:'BudgetName'},
                    {header:'Current',dataKey:'BudgetSpend'},
                    {header:'Forcasted',dataKey:'BudgetForcast'},
                    {header:'Budgeted',dataKey:'Budgeted'}
                    )
        ...
        if (costData && this.props.costByTagDetails.costByTagInProgress) {
            ...
            if (props.costByTagDetails.costByTagDetails.Response.Output.Content.EstimatedSavingCost) {
                ...
                if (props.costByTagDetails.costByTagDetails.Response.Output.Content.EstimatedSavingCost.EstiCost) {
                    ...
                    if (this.state.savingDataList.length == 0) {
                ...
                if(this.state.estCostPdfHeader.length == 0){

                    this.state.estCostPdfHeader.push({header:'Instance Id',dataKey:'instanceid'},
                    {header:'Estimated Savings',dataKey:'saving'},
                    {header:'Account Id',dataKey:'accountid'},
                    {header:'Instance Type',dataKey:'instancetype'},
                    {header:'Recommended Instance Type',dataKey:'Recommendedinstancetype'},
            ...
            if (!this.state.selectedCheckboxes.length) {
                ...
                //console.log(barCostseries + 'llll' + barCostOption[0] + 'zzzz' + `${moment().subtract(2, 'months').startOf('month').add(3, 'days').format('MM-DD-YYYY')}`)
                // if(barCostOption)
    ...
    exportPdfCos() {
        ...
        doc.text("CostOnServices Details", 20, 10)
        doc.autoTable({
            body:this.state.cosCsvFileData,
            columns:this.state.cosPdfHeader
        })
        doc.save('CosDetails.pdf')
    ...
    exportPdfBudget(){
        ...
        doc.text("Budget Details", 20, 10)
        doc.autoTable({
            body:this.state.budgetDataList,
            columns:this.state.budgetPdfHeader
        })
        doc.save('BudgetDetails.pdf')
    ...
    exportPdfEstCost(){
        ...
        doc.text("Estimated Cost Details", 20, 10)
        doc.autoTable({
            body:this.state.savingDataList,
            columns:this.state.estCostPdfHeader
        })
        doc.save('EstimatedCostDetails.pdf')
    }
    ...
    handlePageChange(event,newpage){
     ...
     handleRowsPerPageChange(event) {
        this.setState({
            rowPerPageMui: event.target.value,
            pageMui:0,
        })
    }
    ...
    render() {
        ...
        arrayRechart1.forEach(({ labelCos, cosCost }) => Object.assign(dtaCurMonth, { [labelCos]: cosCost }))
        ...
        arrayRechart2.forEach(({ labelCos, cosCost }) => Object.assign(dtaPreMonth, { [labelCos]: cosCost }))


        const dtaLast3rdMonth = {}
        ...
        arrayRechart3.forEach(({ labelCos, cosCost }) => Object.assign(dtaLast3rdMonth, { [labelCos]: cosCost }))


        const dtaLast4thMonth = {}
        ...
        arrayRechart4.forEach(({ labelCos, cosCost }) => Object.assign(dtaLast4thMonth, { [labelCos]: cosCost }))


        const dtaLast5thMonth = {}
        ...
        arrayRechart5.forEach(({ labelCos, cosCost }) => Object.assign(dtaLast5thMonth, { [labelCos]: cosCost }))


        const dtaLast6thMonth = {}
        ...
        arrayRechart6.forEach(({ labelCos, cosCost }) => Object.assign(dtaLast6thMonth, { [labelCos]: cosCost }))
        ...
        const data3 =[dtaPreMonth,dtaLast3rdMonth,dtaLast4thMonth,dtaLast5thMonth,dtaLast6thMonth,dtaCurMonth]

        const allKeys = [...new Set(data3.flatMap(i=>Object.keys(i)))]
        ...
        if (this.state.cosCsvFileData.length == 0) {
            
            allDtaz.forEach((item) => {
          

                this.state.cosCsvFileData.push({
                    res: item.res,
                    pre: item.pre,
                    last2nd: item.last2nd,
                    last3rd: item.last3rd,
                    last4th: item.last4th,
                    last5th: item.last5th,
                    last6th: item.last6th,
                })
        ...
        // let prez = `${moment().subtract(1,'months').format('MMM YYYY')}`
        // let last2ndz = `${moment().subtract(2,'months').format('MMM YYYY')}`
        // let last3rdz = `${moment().subtract(3,'months').format('MMM YYYY')}`
        // let last4thz = `${moment().subtract(4,'months').format('MMM YYYY')}`
        // let last5thz = `${moment().subtract(5,'months').format('MMM YYYY')}`
        // let last6thz = `${moment().subtract(6,'months').format('MMM YYYY')}`
        if (this.state.cosCsvFileHeader.length == 0) {
            this.state.cosCsvFileHeader.push({label:'#',key:'res'},
                {label:`${moment().subtract(1,'months').format('MMM YYYY')}`,key:'pre'},
                {label:`${moment().subtract(2,'months').format('MMM YYYY')}`,key:'last2nd'},
                {label:`${moment().subtract(3,'months').format('MMM YYYY')}`,key:'last3rd'},
                {label:`${moment().subtract(4,'months').format('MMM YYYY')}`,key:'last4th'},
                {label:`${moment().subtract(5,'months').format('MMM YYYY')}`,key:'last5th'},
                {label:`${moment().subtract(6,'months').format('MMM YYYY')}`,key:'last6th'})
        //     allDtaz.forEach((item) => {
        ...
        if (this.state.cosPdfHeader.length == 0) {
            this.state.cosPdfHeader.push({header:'#',dataKey:'res'},
                {header:`${moment().subtract(1,'months').format('MMM YYYY')}`,dataKey:'pre'},
                {header:`${moment().subtract(2,'months').format('MMM YYYY')}`,dataKey:'last2nd'},
                {header:`${moment().subtract(3,'months').format('MMM YYYY')}`,dataKey:'last3rd'},
                {header:`${moment().subtract(4,'months').format('MMM YYYY')}`,dataKey:'last4th'},
                {header:`${moment().subtract(5,'months').format('MMM YYYY')}`,dataKey:'last5th'},
                {header:`${moment().subtract(6,'months').format('MMM YYYY')}`,dataKey:'last6th'})
        //     allDtaz.forEach((item) => {
        ...
        // console.log(JSON.stringify(this.state.cosPdfHeader)+'cospdf'+this.state.cosPdfHeader.length)
        //   console.log(JSON.stringify(this.state.cosCsvFileData)+'coscsv'+this.state.cosCsvFileData.length)

        // console.log(JSON.stringify(this.state.cosCsvFileHeader)+'cos header')
        ...
        let CurrentMonthCosMOd = `${moment().subtract(6, 'months').startOf('month').format('YYYY-MM-DD')} - ${moment().subtract(6, 'months').endOf('month').add(1, 'days').format('YYYY-MM-DD')}`
        ...
        const CustomTooltip = ({ active, payload, label }) => {
            if (active && payload && payload.length) {
                // const sumValues = obj => Object.values(payload).reduce((a, b) => a + b, 0);
                // sumValues(payload )
                // console.log(sumValues(payload ) + 'recharts')
                // console.log(Object.values(payload).reduce((a, b) => a + b, 0));
                // console.log(JSON.stringify(payload.value) + 'payload')
                let totalval =[]
                payload.map((x)=>{
                    console.log(payload.length+'payload')
                    totalval.push(x.value)
                })
                // console.log(totalval +'payload')
                const sumArrayVal = totalval.reduce((b, a) => b + a, 0);
// console.log(sumArrayVal +'sumArrayVal'); 
                ...
                return (
                    <div className="custom-tooltip" >
                        ...
                        <div style={{ padding: '0 10 10 10', marginBottom: '225px', borderRadius: '4px', backgroundColor: 'rgba(0,0,0,0.6)', zIndex: '111111', width: '700px' }}>
                            <h1 style={{ fontSize: '17px', color: '#fff',padding:'10px', backgroundColor: '#0071fb', textAlign: 'center' }}>{`${label}`} <span className="totalCosBarVal">&#40;Total Cost : {sumArrayVal}$&#41;</span> </h1> 
                            {payload.map((pld) => (
                                ...
                                <div className='CostSaving_lft'>
                                                ...
                                                <Col md={15} lg={15} className="costflex">
                                                    ...
                                                    >
                                                        ...
                                                        {/* <div style={{ marginLeft: '790px', fontSize: 'medium' }}>
                                                         ...
                                                         <div className="findingsDownloads">
                                                      

                                                      <button className="btn btn-primary filter_res"><CSVLink style={{ color: '#fff', textDecoration: 'none' }} {...csvReport}>Download CSV
                                                      </CSVLink></button>
                                                      <span title="Save as PDF">
                                       <PrintIcon onClick={this.exportPdfEstCost.bind(this)} className="pdfIcon"  style={{color: 'rgba(0,0,0,0.6)',marginTop:'3px',marginLeft:'15px' }} />
                                       </span>
                                    ...
                                    <Card sx={{ minWidth: 275 }} style={{ margin: '20px', padding: '0', position: 'relative' }} >
                                        <CardContent>
                                            ...
                                            <div style={{marginBottom: "5px"}}> Note: All values in USD</div>
                                    ...
                                    <CardContent >
                                        ...
                                        <Box>
                                            ...
                                            <Button style={{ marginLeft: '10px' }} onClick={() => { this.handleModal() }} className="btn btn-primary filter">Create Budget</Button>
                                                <Button type="button" style={{marginLeft:'12px'}}
                                                ...
                                                <span title="Save as PDF">
                                             <PrintIcon onClick={this.exportPdfBudget.bind(this)} className="pdfIcon"  style={{color: 'rgba(0,0,0,0.6)',marginTop:'3px' }} />
                                             </span>
                                                ...
                                                <Modal.Body>
                                                    <Col >
                                                        <Form horizontal className="tags-config-form data-supplier-form" >
                                                            ...
                                                            {this.state.typeSelection === 'Custom' && <Col>
                                                                {/* <Col><hr></hr></Col>  */}
                                                                <Col style={{ fontSize: "16px", padding: "10px", backgroundColor: '#f5f5f5' }}> Customized cost budget</Col>
                                                               <div className="customDates">
                                                               <FormGroup style={{ marginTop: '33px' }} controlId="name">
                                                                    <Col componentClass={ControlLabel} sm={3}>
                                                                        <Col className="required-field">Start Date</Col>
                                                                    </Col>
                                                                    <Col sm={9}>
                                                                        <FormControl name="startDate" type="date" defaultValue={this.state.budgetPayloadCustom.StartDate} placeholder="dd-mm-yyyy" 
                                                                              min={`${moment().subtract(6,'months').startOf('month').format('YYYY-MM-DD')}`}
                                                                              max={`${moment().add(6,'months').format('YYYY-MM-DD')}`}
                                                                        onChange={this.updatebudgetStartDate.bind(this)} />
                                                                    </Col>
                                                                </FormGroup>
                                                                <FormGroup style={{ marginTop: '33px' }} controlId="name">
                                                                    <Col componentClass={ControlLabel} sm={3}>
                                                                        <Col className="required-field">End Date</Col>
                                                                    </Col>
                                                                    <Col sm={9}>
                                                                        <FormControl name="endDate" type="date"  defaultValue={this.state.budgetPayloadCustom.EndDate} placeholder="dd-mm-yyyy"
                                                                        min={`${moment().add(1,'months').startOf('month').format('YYYY-MM-DD')}`}
                                                                        max={`${moment().add(6,'months').format('YYYY-MM-DD')}`}
                                                                        onChange={this.updatebudgetEndDate.bind(this)} />
                                                                    </Col>
                                                                </FormGroup>
                                                               </div>
                                                                <FormGroup style={{ marginTop: '33px' }} controlId="name">
                                                                ...
                                                                <FormGroup style={{ marginTop: '33px' }} controlId="email">
                                                                    ...
                                                                    <Col sm={9}>
                                                                        <FormControl name="email" type="text" placeholder="Separate email addresses using commas" defaultValue={this.state.budgetPayloadCustom.Email} onKeyUp={this.updatebudgetemailData} />
                                                                    </Col>
                                                                </FormGroup>
                                                            </Col>}
                                            ...
                                            <TableContainer component={Paper}>
                                                <Table sx={{ minWidth: 700 }} aria-label="customized table">
                                                    ...
                                                    <TableBody>
                                                        {rowBudgetTable.slice(this.state.pageMui * this.state.rowPerPageMui, this.state.pageMui * this.state.rowPerPageMui + this.state.rowPerPageMui).map((row) => (

                                                            <StyledTableRow key={row.name}>
                                                                {/* <StyledTableCell component="th" scope="row">
                <input type='checkbox'/> &nbsp; 
              </StyledTableCell> */}
                                                                <StyledTableCell className='budget_table' style={{backgroundColor:row.BudgetSpend>row.Budgeted?'#ffc0bd':''}}>{row.BudgetName}</StyledTableCell>
                                                                {/* <StyledTableCell style={{paddingLeft:'75px'}} className='budget_table'>{row.fat}</StyledTableCell> */}
                                                                <StyledTableCell className='budget_table' style={{backgroundColor:row.BudgetSpend>row.Budgeted?'#ffc0bd':''}}>{row.BudgetSpend} &nbsp; {row.BudgetSpendUnit}</StyledTableCell>
                                                                <StyledTableCell className='budget_table' style={{backgroundColor:row.BudgetSpend>row.Budgeted?'#ffc0bd':''}}>{row.BudgetForcast}&nbsp;{row.BudgetForcastUnit}</StyledTableCell>
                                                                <StyledTableCell className='budget_table' style={{backgroundColor:row.BudgetSpend>row.Budgeted?'#ffc0bd':''}} >{row.Budgeted}&nbsp;{row.BudgetedUnit}</StyledTableCell>
                                        ...
                                        page={this.state.pageMui}
                                        rowsPerPage={this.state.rowPerPageMui}
                                        component="div"
                                        count={rowBudgetTable.length}
                                        onPageChange={this.handlePageChange.bind(this)}
                                        onRowsPerPageChange={this.handleRowsPerPageChange.bind(this)}
                                        >
                                    </TablePagination>
                                    <div style={{display:'flex',justifyContent:'end', alignContent:'center',marginTop:'5px'}}> <span><img className='tableRedCol' src={tableRed} alt='table_indication'/>  Indicates the budget exceeded</span></div>
                                        </Box>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        ...
                                        <div style={{ display:'flex',alignItems:'center'}}>
                                            <CSVLink
                                                ...
                                                data={this.state.cosCsvFileData}
                                                headers={this.state.cosCsvFileHeader}
                                                filename="CosReport.csv"
                                                target="_blank"
                                            >   <Button type="button"
                                                className="btn btn-primary filter_res"
                                            ...
                                            >Download CSV</Button></CSVLink>                                                
                                             <span title="Save as PDF">
                                             <PrintIcon onClick={this.exportPdfCos.bind(this)} className="pdfIcon"  style={{color: 'rgba(0,0,0,0.6)' }} />
                                             </span>
                                        </div>
                                        ...
                                        <Table aria-label="customized table">
                                            <TableHead className='budgetReport_table'>
                                                <TableRow >
                                                    ...
                                                    <StyledTableCell style={{ paddingLeft: '30px' }} >{`${moment().subtract(1,'Months').format('MMM YYYY')}`}</StyledTableCell>
                                                    <StyledTableCell style={{ paddingLeft: '30px' }} >{`${moment().subtract(2,'Months').format('MMM YYYY')}`}</StyledTableCell>

                                                    <StyledTableCell style={{ paddingLeft: '30px' }} >{`${moment().subtract(3,'Months').format('MMM YYYY')}`}</StyledTableCell>
                                                    <StyledTableCell style={{ paddingLeft: '30px' }} >{`${moment().subtract(4,'Months').format('MMM YYYY')}`}</StyledTableCell>
                                                    <StyledTableCell style={{ paddingLeft: '30px' }} >{`${moment().subtract(5,'Months').format('MMM YYYY')}`}</StyledTableCell>
                                                    <StyledTableCell style={{ paddingLeft: '30px' }} >{`${moment().subtract(6,'Months').format('MMM YYYY')}`}</StyledTableCell>
                                            ...
                                            <TableBody>
                                                ...
                                                {/* {rowModified.map((row, i) => (
                                                    <StyledTableRow key={i}>

                                                        <StyledTableCell >{row.labelCos}</StyledTableCell>
                                                        <StyledTableCell style={{ paddingLeft: '5px', }} >{row.cosCost}</StyledTableCell>


                                                    </StyledTableRow>
                                                ))} */}

                                                {allDtaz.slice(this.state.pageMui * this.state.rowPerPageMui, this.state.pageMui * this.state.rowPerPageMui + this.state.rowPerPageMui).map((row, i) => (
                                                    <StyledTableRow key={i}>

                                                        <StyledTableCell >{row.res}</StyledTableCell>
                                                        <StyledTableCell style={{ paddingLeft: '5px', }} >{row.pre}</StyledTableCell>
                                                        <StyledTableCell style={{ paddingLeft: '5px', }} >{row.last2nd}</StyledTableCell>
                                                        <StyledTableCell style={{ paddingLeft: '5px', }} >{row.last3rd}</StyledTableCell>
                                                        <StyledTableCell style={{ paddingLeft: '5px', }} >{row.last4th}</StyledTableCell>
                                                        <StyledTableCell style={{ paddingLeft: '5px', }} >{row.last5th}</StyledTableCell>
                                                        <StyledTableCell style={{ paddingLeft: '5px', }} >{row.last6th}</StyledTableCell>
                                            ...
                                            <span className="modCosBlk">
                                                ...
                                                <div className="modCosCol">
                                                    <ul>
                                                        ...
                                                        <span className="modCosHeader"><li>{moment().subtract(6, 'months').format('MMM YYYY')}</li></span>
